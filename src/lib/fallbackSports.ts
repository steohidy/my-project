...
// ==================== API RAPIDAPI (SPORTAPI7) ====================

/**
 * Tente de récupérer les données depuis RapidAPI
 */
  ...
  try {
    const response = await fetch(
      `https://${RAPIDAPI_HOST}${endpoint}`,
      {
        ...
        next: { revalidate: 7200 }, // 2 heures cache
      }
    );

    if (!response.ok) {
      console.log(`⚠️ RapidAPI ${endpoint}: Status ${response.status}`);
      return [];
    }
    ...
    console.error(`Erreur RapidAPI ${endpoint}:`, error);
    return [];
  }
}
...
async function fetchRapidAPIFootball(): Promise<FallbackMatch[]> {
  ...
  // Vérifier le cache
  if (cache.football.data.length > 0 && (now - cache.football.timestamp) < CACHE_TTL) {
...
/**
 * Récupère les matchs NBA depuis RapidAPI
 */
async function fetchRapidAPIBasketball(): Promise<FallbackMatch[]> {
  const now = Date.now();
  
  if (cache.basketball.data.length > 0 && (now - cache.basketball.timestamp) < CACHE_TTL) {
    return cache.basketball.data;
  }
  
  const events = await fetchFromRapidAPI('/basketball/live');
  
  if (events.length === 0) {
    const generated = generateNBAMatches();
    cache.basketball = { data: generated as any, timestamp: now };
    return generated;
  }
  
  const matches = events.slice(0, 8).map((event: any) => ({
    id: event.id || `rapid_nba_${Date.now()}`,
    homeTeam: event.homeTeam?.name || event.home,
    awayTeam: event.awayTeam?.name || event.away,
    sport: 'Basket' as const,
    league: 'NBA',
    date: (event.startTime || event.date || new Date().toISOString()).split('T')[0],
    time: (event.startTime || event.date || '').split('T')[1]?.substring(0, 5) || '01:00',
    oddsHome: event.odds?.home || 1.90,
    oddsDraw: null,
    oddsAway: event.odds?.away || 1.90,
    status: 'upcoming' as const,
  ...
  cache.basketball = { data: matches as any, timestamp: now };
  return matches;
}

/**
 * Récupère les matchs NHL depuis RapidAPI
 */
async function fetchRapidAPIHockey(): Promise<FallbackMatch[]> {
  const now = Date.now();
  
  if (cache.hockey.data.length > 0 && (now - cache.hockey.timestamp) < CACHE_TTL) {
    return cache.hockey.data;
  }
  
  const events = await fetchFromRapidAPI('/hockey/live');
  
  if (events.length === 0) {
    const generated = generateNHLMatches();
    cache.hockey = { data: generated as any, timestamp: now };
    return generated;
  }
  
  const matches = events.slice(0, 6).map((event: any) => ({
    id: event.id || `rapid_nhl_${Date.now()}`,
    homeTeam: event.homeTeam?.name || event.home,
    awayTeam: event.awayTeam?.name || event.away,
    ...
    time: (event.startTime || event.date || '').split('T')[1]?.substring(0, 5) || '01:00',
    oddsHome: event.odds?.home || 1.90,
    oddsDraw: event.odds?.draw || 3.80,
    oddsAway: event.odds?.away || 1.90,
    status: 'upcoming' as const,
    source: 'RapidAPI' as const,
    winProb: { home: 45, draw: 22, away: 33 },
    confidence: 'medium' as const,
    riskPercentage: 48,
  }));
  
  cache.hockey = { data: matches as any, timestamp: now };
  return matches;
}
