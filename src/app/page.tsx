...
export interface Match {
  ...
  minute?: string; // Pour le football
  period?: number; // Pour le basket (Q1, Q2, Q3, Q4)
  clock?: string; // Chrono basket
  homeRecord?: string; // Record NBA équipe domicile
  awayRecord?: string; // Record NBA équipe extérieur
  insight: {
...
function NFLSection() {
  const [nflMatches, setNflMatches] = useState<NFLMatch[]>([]);
  ...
  useEffect(() => {
    const fetchNFL = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/nfl-pro');
        const data = await response.json();
  ...
  const valueBets = nflMatches.filter(m => m.insights?.moneyline?.valueBet?.detected);
  const highConfidence = nflMatches.filter(m => (m.insights?.confidence || 0) >= 70);
  ...
  const getMatchTimeRange = () => {
    if (nflMatches.length === 0) return null;
    
    const times = nflMatches.map(m => m.time).filter(Boolean).sort();
    if (times.length === 0) return null;
    
    return `${times[0]} - ${times[times.length - 1]}`;
  };
  
  const timeRange = getMatchTimeRange();
    ...
    <div style={{ marginBottom: '12px' }}>
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {timeRange && (
            <p style={{ color: '#22c55e', fontSize: '10px', fontWeight: 'bold' }}>
              🕐 {timeRange}
            </p>
          )}
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#eab308' }}>
                {nflMatches.length > 0 ? (nflMatches.reduce((acc, m) => acc + (m.projected?.totalPoints || 0), 0) / nflMatches.length).toFixed(1) : '0'}
              </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {filteredMatches.map((match, idx) => (
            <NFLMatchCard key={match.id} match={match} index={idx + 1} />
          ))}
        </div>
      )}
    </div>
...
function NFLMatchCard({ match, index }: { match: NFLMatch; index: number }) {
  const isHomeFavorite = match.projected.homeWinProb > match.projected.awayWinProb;
  const winnerTeam = isHomeFavorite ? match.homeTeam : match.awayTeam;
  const winnerAbbr = isHomeFavorite ? match.homeAbbr : match.awayAbbr;
  const winnerProb = isHomeFavorite ? match.projected.homeWinProb : match.projected.awayWinProb;
  
  const confidenceColor = match.insights.confidence >= 70 ? '#22c55e' 
    : match.insights.confidence >= 50 ? '#f97316' : '#ef4444';
  
  const hasValueBet = match.insights.moneyline?.valueBet?.detected;
  const valueEdge = match.insights.moneyline?.valueBet?.edge || 0;

  // Interprétation des métriques
  const dvoaDiff = match.factors?.dvoaDiff || 0;
  const epaDiff = match.factors?.epaDiff || 0;
  const turnoverEdge = match.factors?.turnoverEdge || 0;
  const restEdge = match.factors?.restEdge || 0;
  const injuryEdge = match.factors?.injuryEdge || 0;
  
  // Score projeté formaté
  const homeScore = Math.round(match.projected.homePoints);
  const awayScore = Math.round(match.projected.awayPoints);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: '10px', color: '#888' }}>
            🏠 Domicile {match.homeRecord && `(${match.homeRecord})`}
          </div>
          <div style={{
            marginTop: '6px',
            fontSize: '18px',
        ...
        <div style={{
          ...
          <div style={{ fontSize: '10px', color: '#888' }}>
            ✈️ Extérieur {match.awayRecord && `(${match.awayRecord})`}
          </div>
          <div style={{
            marginTop: '6px',
            fontSize: '18px',
      ...
      <div style={{
        background: `linear-gradient(135deg, ${isHomeFavorite ? '#22c55e15' : '#3b82f615'} 0%, #0a0a0a 100%)`,
        borderRadius: '10px',
        padding: '12px',
        marginBottom: '12px',
        border: `1px solid ${isHomeFavorite ? '#22c55e30' : '#3b82f630'}`
      }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            {match.insights.spread.recommendation === 'home' ? `✅ ${match.homeAbbr} couvre` 
              : match.insights.spread.recommendation === 'away' ? `✅ ${match.awayAbbr} couvre` 
              : '⏳ Éviter'}
          </div>
          <div style={{ fontSize: '9px', color: '#666' }}>
            {match.insights.spread.line !== 0 ? `Ligne: ${match.insights.spread.line > 0 ? '-' : '+'}${Math.abs(match.insights.spread.line)}` : 'Match serré'}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            fontSize: '14px',
            fontWeight: 'bold',
            color: match.insights.total?.recommendation === 'over' ? '#22c55e' 
              : match.insights.total?.recommendation === 'under' ? '#ef4444' : '#888'
          }}>
            {match.insights.total?.recommendation === 'over' ? '⬆️ Parier Over' 
              : match.insights.total?.recommendation === 'under' ? '⬇️ Parier Under' 
              : '⏳ Éviter'} {match.insights.total?.line}
          </div>
          <div style={{ fontSize: '9px', color: '#666' }}>
            Prédit: {match.insights.total?.predicted || match.projected.totalPoints} pts
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '13px', fontWeight: 'bold', color: dvoaDiff > 0 ? '#22c55e' : dvoaDiff < 0 ? '#ef4444' : '#888' }}>
            {dvoaDiff > 0 ? `✅ ${match.homeAbbr} +${dvoaDiff.toFixed(1)}%` 
              : dvoaDiff < 0 ? `✅ ${match.awayAbbr} +${Math.abs(dvoaDiff).toFixed(1)}%` 
              : '⚖️ Égal'}
          </div>
          <div style={{ fontSize: '9px', color: '#666' }}>
            Avantage global
        ...
        <div style={{
          ...
          <div style={{ fontSize: '13px', fontWeight: 'bold', color: epaDiff > 0 ? '#22c55e' : epaDiff < 0 ? '#ef4444' : '#888' }}>
            {epaDiff > 0 ? `✅ ${match.homeAbbr} +${epaDiff.toFixed(2)}` 
              : epaDiff < 0 ? `✅ ${match.awayAbbr} +${Math.abs(epaDiff).toFixed(2)}` 
              : '⚖️ Égal'}
          </div>
          <div style={{ fontSize: '9px', color: '#666' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '13px', fontWeight: 'bold', color: turnoverEdge > 0 ? '#22c55e' : turnoverEdge < 0 ? '#ef4444' : '#888' }}>
            {turnoverEdge > 0 ? `✅ ${match.homeAbbr} +${turnoverEdge}` 
              : turnoverEdge < 0 ? `✅ ${match.awayAbbr} +${Math.abs(turnoverEdge)}` 
              : '⚖️ Égal'}
          </div>
          <div style={{ fontSize: '9px', color: '#666' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '13px', fontWeight: 'bold', color: (restEdge > 0 || injuryEdge > 0) ? '#22c55e' : (restEdge < 0 || injuryEdge < 0) ? '#ef4444' : '#888' }}>
            {(restEdge > 0 || injuryEdge > 0) ? `✅ ${match.homeAbbr} avantagé` 
              : (restEdge < 0 || injuryEdge < 0) ? `✅ ${match.awayAbbr} avantagé` 
              : '⚖️ Équilibré'}
          </div>
          <div style={{ fontSize: '9px', color: '#666' }}>
      ...
      {/* Injury Report */}
      {match.injuryReport && (match.injuryReport.home?.keyPlayersOut?.length > 0 || match.injuryReport.away?.keyPlayersOut?.length > 0) && (
        <div style={{
          background: 'rgba(239, 68, 68, 0.1)',
          border: '1px solid rgba(239, 68, 68, 0.3)',
          borderRadius: '8px',
...
// ===== SECTION NHL HOCKEY =====
function NHLSection({ matches, loading, lastUpdate }: { matches: Match[]; loading: boolean; lastUpdate: Date }) {
  ...
  return (
    <div style={{ marginBottom: '12px' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {displayedMatches.slice(0, 15).map((match, idx) => (
            <NHLMatchCard key={match.id} match={match} index={idx + 1} />
          ))}
        </div>
      )}
    </div>
...
function NHLMatchCard({ match, index }: { match: Match; index: number }) {
  ...
  const isHomeFavorite = match.oddsHome < match.oddsAway;
  const winnerProb = isHomeFavorite 
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          background: `${isHomeFavorite ? '#06b6d4' : '#3b82f6'}20`,
          padding: '4px 8px',
          borderRadius: '4px',
          fontSize: '10px',
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* SCORE */}
          {(isLive || isFinished) && awayScore !== undefined ? (
            <div style={{
              marginTop: '6px',
              fontSize: '28px',
              fontWeight: 'bold',
      ...
      {match.nhlPredictions && !isLive && !isFinished && (
        <>
          ...
          {/* Value Bet */}
          {match.nhlPredictions.valueBet?.detected && (
            <div style={{
              background: 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)',
              borderRadius: '8px',
              padding: '10px',
              ...
              <div>
                ...
                <div style={{ fontSize: '13px', fontWeight: 'bold', color: '#fff' }}>
                  {match.nhlPredictions.valueBet.type === 'home' ? match.homeTeam : 
                   ...
                   match.nhlPredictions.valueBet.type?.toUpperCase()}
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
...
function ApiStatusSection() {
  ...
  const [europaMatches, setEuropaMatches] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  const loadApiStatus = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/real-odds');
      const data = await res.json();
      setApiStatus(data);
    } catch (e) {
      console.error('Erreur chargement statut API:', e);
  ...
  const refreshOdds = async () => {
    ...
    try {
      const res = await fetch('/api/real-odds', { method: 'POST' });
      const data = await res.json();
      setApiStatus(data);
      setMessage(`✅ ${data.matches?.length || 0} matchs mis à jour`);
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
              ...
              <div style={{ background: '#0a0a0a', borderRadius: '6px', padding: '10px', textAlign: 'center' }}>
                <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#3b82f6' }}>
                  {apiStatus.stats?.active || apiStatus.matches?.length || 0}
                </div>
                <div style={{ fontSize: '9px', color: '#888' }}>Matchs</div>
              </div>
            </div>
      ...
      <div style={{ 
        ...
        {europaMatches.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '200px', overflowY: 'auto' }}>
            {europaMatches.slice(0, 10).map((match, i) => (
              <div key={match.id || i} style={{ 
                background: '#0a0a0a', 
                borderRadius: '6px', 
                padding: '8px 10px',
                display: 'flex',
            ...
            {europaMatches.length > 10 && (
              <div style={{ textAlign: 'center', color: '#666', fontSize: '10px', padding: '4px' }}>
        ...
        {europaMatches.length === 0 && !loadingEuropa && (
...
function TennisSection() {
  const [predictions, setPredictions] = useState<TennisPrediction[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetchTennis();
    const interval = setInterval(fetchTennis, 5 * 60 * 1000);
  ...
  const filteredPredictions = activeFilter === 'recommended'
    ? predictions.filter(p => p.betting.recommendedBet)
    : activeFilter === 'atp'
    ? predictions.filter(p => p.category === 'atp')
    : activeFilter === 'wta'
    ? predictions.filter(p => p.category === 'wta')
    : predictions;
  ...
  return (
    <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {Object.entries(stats.bySurface).map(([surface, count]) => (
                <span key={surface} style={{
                  background: `${surfaceColors[surface]}20`,
          ...
          <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #333' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {Object.entries(stats.byConfidence).map(([conf, count]) => (
                <span key={conf} style={{
                  background: `${confidenceColors[conf]}20`,
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '12px', flexWrap: 'wrap' }}>
        ...
        >
          💰 Recommandés ({stats?.recommendedBets || 0})
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {filteredPredictions.map((prediction, idx) => (
            <TennisPredictionCard key={prediction.matchId} prediction={prediction} index={idx + 1} surfaceColors={surfaceColors} surfaceLabels={surfaceLabels} confidenceColors={confidenceColors} confidenceLabels={confidenceLabels} />
          ))}
        </div>
      )}
    </div>
...
function TennisPredictionCard({ prediction, index, surfaceColors, surfaceLabels, confidenceColors, confidenceLabels }: { 
  ...
  const isPlayer1Favorite = prediction.odds1 < prediction.odds2;
  const predictedPlayer = pred.winner === 'player1' ? prediction.player1 : prediction.player2;
  const winnerOdds = pred.winner === 'player1' ? prediction.odds1 : prediction.odds2;
  
  const confidenceColor = confidenceColors[pred.confidence] || '#eab308';
  const confidenceLabel = confidenceLabels[pred.confidence] || 'Moyenne';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ 
            ...
            background: `${surfaceColors[prediction.surface]}20`,
            padding: '2px 6px',
            borderRadius: '4px',
            fontWeight: 'bold'
          }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: '10px', color: '#888' }}>
            Cote: <span style={{ color: '#a855f7', fontWeight: 'bold' }}>{prediction.odds1?.toFixed(2) || '1.85'}</span>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '10px', color: '#888' }}>
            Cote: <span style={{ color: '#3b82f6', fontWeight: 'bold' }}>{prediction.odds2?.toFixed(2) || '1.85'}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        {/* Betting info */}
        {prediction.betting.recommendedBet && (
          <div style={{ 
            marginTop: '8px', 
            padding: '8px', 
            background: '#22c55e15', 
      ...
      {/* Warnings */}
      {prediction.warnings.length > 0 && expanded && (
        <div style={{ marginBottom: '10px', padding: '8px', background: '#ef444410', borderRadius: '6px' }}>
          {prediction.warnings.map((warning, i) => (
            <div key={i} style={{ fontSize: '9px', color: '#ef4444' }}>⚠️ {warning}</div>
          ))}
        </div>
      )}
...
// ===== SECTION FOOTBALL =====
function FootballSection({ matches, loading, lastUpdate }: { matches: Match[]; loading: boolean; lastUpdate: Date }) {
  const [activeTab, setActiveTab] = useState<'safes' | 'moderate' | 'risky' | 'live' | 'finished' | 'all'>('all');

  // Catégoriser les matchs
  const now = new Date();
  const getMatchStatus = (match: Match): 'upcoming' | 'live' | 'finished' => {
    const matchDate = new Date(match.date);
    const matchDurationMs = 2 * 60 * 60 * 1000;
    const matchEndTime = new Date(matchDate.getTime() + matchDurationMs);
    if (now < matchDate) return 'upcoming';
    if (now >= matchDate && now <= matchEndTime) return 'live';
    return 'finished';
  };

  const upcomingMatches = matches.filter(m => getMatchStatus(m) === 'upcoming');
  const liveMatches = matches.filter(m => getMatchStatus(m) === 'live' || m.isLive);
  const finishedMatches = matches.filter(m => getMatchStatus(m) === 'finished' || m.isFinished);
  ...
  return (
    <div style={{ marginBottom: '12px' }}>
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '12px', flexWrap: 'nowrap', overflowX: 'auto' }}>
        <TabButtonCompact active={activeTab === 'all'} onClick={() => setActiveTab('all')} icon="📋" label="Tous" count={activeMatches.length} />
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {displayedMatches.slice(0, 10).map((match, index) => (
...
// ===== SECTION BASKETBALL =====
function BasketballSection({ matches, loading, lastUpdate }: { matches: Match[]; loading: boolean; lastUpdate: Date }) {
  const [activeTab, setActiveTab] = useState<'safes' | 'live' | 'finished' | 'all'>('safes');
  
  // Vérifier créneau horaire NBA
  const hour = new Date().getUTCHours();
  const isNBATime = [0, 1, 2, 3, 19, 20, 21, 22, 23].includes(hour);
  ...
  if (!isNBATime && matches.filter(m => !m.isFinished).length === 0) {
    return (
      <div style={{ marginBottom: '12px' }}>
...
function AppDashboard({ onLogout, userInfo }: { onLogout: () => void; userInfo: UserInfo | null }) {
  ...
  const [activeSection, setActiveSection] = useState<'football' | 'basketball' | 'nhl' | 'nfl' | 'tennis' | 'expert' | 'analyse' | 'antitrap' | 'bankroll' | 'results' | 'pronostiqueur' | 'admin' | 'apistatus'>('football');
  ...
  const savePredictionsToDB = async (matchList: Match[]) => {
    try {
      const predictions = matchList
        .filter(m => m && m.insight) // Filtrer les matchs sans insight
        .map(m => ({
          matchId: m.id,
          homeTeam: m.homeTeam,
          awayTeam: m.awayTeam,
  ...
  const getMatchStatus = (match: Match): 'upcoming' | 'live' | 'finished' => {
    const matchDate = new Date(match.date);
    const matchDurationMs = match.sport === 'Basket' ? 2.5 * 60 * 60 * 1000 : 2 * 60 * 60 * 1000; // 2.5h ou 2h
    const matchEndTime = new Date(matchDate.getTime() + matchDurationMs);
  ...
  return (
    <div style={{
      ...
      <aside style={{
        ...
        <NavButton icon="⚽" label="Football" active={activeSection === 'football'} onClick={() => setActiveSection('football')} color="#22c55e" />
      ...
      <main style={{ flex: 1, overflow: 'auto', padding: '12px' }}>
        ...
        {activeSection === 'football' && (
          <FootballSection matches={matches.filter(m => m.sport === 'Foot' || m.sport === 'Football')} loading={loading} lastUpdate={lastUpdate} />
        )}

        {/* Section Basketball */}
        {activeSection === 'basketball' && (
          <BasketballSection matches={matches.filter(m => m.sport === 'Basket' || m.sport === 'Basketball')} loading={loading} lastUpdate={lastUpdate} />
        )}
        ...
        {activeSection === 'expert' && (
          <ExpertAdviceSection matches={matches} />
        )}
        ...
        {activeSection === 'apistatus' && userInfo?.role === 'admin' && (
          <ApiStatusSection />
        )}
...
function NBAMatchCard({ match, index }: { match: Match; index: number }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: '12px',
      padding: '14px',
      border: isLive ? '2px solid #ef4444' : `1px solid ${isHomeFavorite ? '#f9731660' : '#3b82f660'}`,
      marginBottom: '10px',
      ...
      <div style={{
        ...
        <div style={{
          background: isLive ? '#ef444420' : `${riskColor}20`,
          padding: '4px 8px',
          borderRadius: '4px',
          fontSize: '9px',
...
function FootballMatchCard({ match, index }: { match: Match; index: number }) {
  ...
  const now = new Date();
  const matchDate = new Date(match.date);
  const matchDurationMs = 2 * 60 * 60 * 1000; // 2h pour le foot
  const matchEndTime = new Date(matchDate.getTime() + matchDurationMs);
  
  // Priorité: API > calcul local
  const isLive = match.isLive ?? (now >= matchDate && now <= matchEndTime);
  const isFinished = match.isFinished ?? now > matchEndTime;
  ...
  return (
    <div style={{
      ...
      borderRadius: '10px',
      padding: '12px',
      border: isLive ? '1px solid #ef4444' : isFinished ? '1px solid #333' : `1px solid ${riskColor}30`,
      marginBottom: '8px',
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 'bold', color: '#fff', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {isFinished && homeScore !== undefined && awayScore !== undefined && (
              <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#888', fontFamily: 'monospace' }}>
                ({homeScore} - {awayScore})
              </span>
            )}
        ...
        {/* Risk Percentage */}
        <div style={{ textAlign: 'right' }}>
          <div style={{ color: riskColor, fontSize: '12px', fontWeight: 'bold' }}>{match.insight?.riskPercentage ?? 50}%</div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            Risque: {match.insight?.riskPercentage ?? 50}%
          </div>
          ...
          <div style={{
            ...
            Confiance: 
            <span style={{
              color: (match.insight?.confidence ?? "medium") === 'high' ? '#22c55e' : (match.insight?.confidence ?? "medium") === 'medium' ? '#f97316' : '#ef4444'
            }}>
              {(match.insight?.confidence ?? "medium") === 'high' ? '⬛⬛⬛' : (match.insight?.confidence ?? "medium") === 'medium' ? '⬛⬛⬜' : '⬛⬜⬜'}
            </span>
          </div>
...
function PronostiqueurProSection() {
  const [predictions, setPredictions] = useState<ProPrediction[]>([]);
  const [allPicks, setAllPicks] = useState<ProPick[]>([]);
  const [stats, setStats] = useState<ProStats | null>(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'safe' | 'fun' | 'combo' | 'history'>('safe');
  const [history, setHistory] = useState<ProPrediction[]>([]);
  ...
  const fetchProData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/pronostiqueur-pro?action=predictions');
      const data = await res.json();
      setPredictions(data.predictions || []);
      setAllPicks(data.allPicks || []);
      setStats(data.stats || null);
    } catch (e) {
      console.error('Erreur chargement pronostiqueur pro:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch('/api/pronostiqueur-pro?action=history');
      const data = await res.json();
  ...
  const updateResult = async (predictionId: string, result: 'won' | 'lost') => {
    try {
      await fetch('/api/pronostiqueur-pro', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const filteredPredictions = predictions.filter(p => p.type === activeTab);
  ...
  return (
    <div style={{ marginBottom: '12px' }}>
      ...
      {/* Onglets */}
      <div style={{ display: 'flex', gap: '6px', marginBottom: '12px', flexWrap: 'wrap' }}>
        {(['safe', 'fun', 'combo', 'history'] as const).map(tab => (
          <button
            key={tab}
            ...
            style={{
              padding: '8px 14px',
              borderRadius: '6px',
              border: activeTab === tab ? `1px solid ${typeColors[tab as 'safe' | 'fun' | 'combo'] || '#888'}` : '1px solid #333',
              background: activeTab === tab ? (typeColors[tab as 'safe' | 'fun' | 'combo'] || '#666') : 'transparent',
          ...
          >
            {typeLabels[tab as 'safe' | 'fun' | 'combo'] || '📜 Historique'}
          </button>
        ))}
      </div>
      ...
      <div style={{
        ...
        {typeDescriptions[activeTab as 'safe' | 'fun' | 'combo'] || 'Historique complet des prédictions avec résultats'}
      </div>
          ...
          {history.length === 0 ? (
            ...
            history.map((pred, idx) => (
              <div key={pred.id} style={{
                ...
                borderRadius: '10px',
                padding: '12px',
                border: `1px solid ${pred.result === 'won' ? '#22c55e30' : pred.result === 'lost' ? '#ef444430' : '#333'}`
              }}>
                ...
                <div style={{ fontSize: '10px', color: '#888' }}>
                  {pred.picks.map(p => `${p.homeTeam} vs ${p.awayTeam}`).join(' + ')}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px', fontSize: '10px' }}>
                  ...
                  <span style={{ color: pred.result === 'won' ? '#22c55e' : pred.result === 'lost' ? '#ef4444' : '#888' }}>
                    {pred.result === 'won' ? `+${pred.potentialWin - pred.totalStake}€` : 
                     pred.result === 'lost' ? `-${pred.totalStake}€` : 
                     `Potentiel: +${pred.potentialWin}€`}
                  </span>
                </div>
                {pred.result === 'pending' && (
                  <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                    <button
                      onClick={() => updateResult(pred.id, 'won')}
                      style={{
                        flex: 1,
                        padding: '6px',
                        borderRadius: '4px',
                    ...
                    <button
                      onClick={() => updateResult(pred.id, 'lost')}
                      style={{
                        flex: 1,
                        padding: '6px',
                        borderRadius: '4px',
          ...
          {filteredPredictions.length === 0 ? (
            ...
            filteredPredictions.map((pred, idx) => (
              <div key={pred.id} style={{
                ...
                borderRadius: '12px',
                padding: '14px',
                border: `1px solid ${typeColors[pred.type]}30`,
                boxShadow: `0 4px 20px ${typeColors[pred.type]}15`
              }}>
                {/* Header */}
                <div style={{
                  display: 'flex',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{
                      background: `${typeColors[pred.type]}20`,
                ...
                {/* Reasoning */}
                {pred.picks[0]?.reasoning && pred.picks[0].reasoning.length > 0 && (
                  <div style={{ marginTop: '10px' }}>
                    <div style={{ fontSize: '10px', color: '#666', marginBottom: '4px' }}>💡 Facteurs clés</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                      {pred.picks.flatMap(p => p.reasoning).slice(0, 4).map((reason, i) => (
                        <span key={i} style={{
                          background: '#1a1a1a',
                          padding: '2px 6px',
                          borderRadius: '4px',
...
function MatchAnalysisSection({ username, matches }: { username: string; matches: Match[] }) {
  ...
  const analyzeMatch = async () => {
    ...
    try {
      ...
      // ===== NOUVEAU: Si pas de match trouvé, utiliser l'API Expert =====
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (expertData && expertData.recommendation) {
              // Construire le résultat à partir de l'expert
              const analysisResult = {
                match: {
                  ...
                  oddsHome: expertData.oddsAnalysis?.favoriteOdds || 2.0,
                  oddsDraw: null,
                  oddsAway: expertData.oddsAnalysis?.favoriteOdds || 2.0,
                  insight: {
                    riskPercentage: 100 - (expertData.recommendation?.expectedValue || 0),
                    confidence: expertData.recommendation?.confidence || 'medium',
                    valueBetDetected: expertData.oddsAnalysis?.isValueBet || false,
                    valueBetType: null
                  }
                },
                similarity: 0,
                ...
                probability: {
                  home: expertData.oddsAnalysis?.impliedProbability || 33,
                  draw: 28,
                  away: 100 - (expertData.oddsAnalysis?.impliedProbability || 33) - 28
                },
                favorite: {
                  team: expertData.oddsAnalysis?.favorite || homeTeam,
                  probability: expertData.oddsAnalysis?.estimatedProbability || 40,
                  odds: expertData.oddsAnalysis?.favoriteOdds || 2.0
                },
                recommendation: expertData.recommendation?.bet === 'avoid' 
                  ? 'ÉVITER CE MATCH' 
                  : `EXPERT: ${expertData.recommendation?.bet === 'home' ? `VICTOIRE ${homeTeam}` : 
                     expertData.recommendation?.bet === 'away' ? `VICTOIRE ${awayTeam}` : 'NUL'}`,
                betType: expertData.recommendation?.reasoning?.[0] || 'Analyse ML personnalisée',
                goals: null,
                cards: null,
                corners: null,
                risk: 100 - (expertData.recommendation?.expectedValue || 0),
                confidence: expertData.recommendation?.confidence || 'medium',
          ...
          console.error('Erreur analyse expert:', expertError);
        }
      ...
      const analysisResult = {
        ...
        risk: match.insight?.riskPercentage ?? 50,
